...
import {
    ...
    MEDIUM_DIALOG_CONFIG,
    NOTION_DIALOG_CONFIG,
} from '../utils';
...
export class DisplayDialogService implements IDisplayDialogService {
    ...
    private lazzyLoadFolderDatabase$ = from(
        import(
            '@modules/folder/folder-database/folder-database.component'
        ).then((component) => component.FolderDatabaseComponent)
    );
    ...
    public open(element: DialogElement): void {
        ...
        const activeFolders = this.fileExplorer
            .activeFolders()()
            .filter((af) => af.id === element.id);
        const currentFolder = this.fileExplorer
            .activeFolders()()
            .find((f) => f.id === element.id);
        const activeFiles = this.fileExplorer
            .activeFiles()()
            .filter((af) => af.id === element.id);
        const currentFile = this.fileExplorer
            .activeFiles()()
            .find((f) => f.id === element.id);
    ...
    openAplication(element: SystemElement, data: any) {
        switch (element.name.toLocaleLowerCase()) {
            case 'medium':
                this.dialog.open(MediumDialogComponent, {
                    ...MEDIUM_DIALOG_CONFIG,
                    data,
                });
                break;
            case 'notion':
                this.dialog.open(NotionDialogComponent, {
                    ...NOTION_DIALOG_CONFIG,
                    data,
                });
                break;
