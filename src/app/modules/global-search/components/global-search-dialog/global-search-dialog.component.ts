...
@Component({
    ...
    imports: [CommonModule, MatIconModule, FocusDialogDirective],
...
export class GlobalSearchDialogComponent {
    public dialogRef: MatDialogRef<GlobalSearchDialogComponent> =
        inject(MatDialogRef);

    private fileExplorerService = inject(FileExplorerService);
    private elements =
        this.fileExplorerService._systemFiles()[0].children![0].children || [];
    ...
    public favoriteElements = computed(() =>
        this.fileExplorerService.favorites()
    );
    ...
    handleKeydown(event: KeyboardEvent) {
        if (event.key === 'ArrowDown') {
            this.selectedIndex =
                (this.selectedIndex + 1) % this.limitElements();
